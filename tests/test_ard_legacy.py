...
ard = load("release_ard_legacy", "notations/ARD-legacy/ard.py")
...
def main():
    ...
    for i, g in enumerate(samples):
        ...
        cases.append({"notation": "ard_legacy", "id": i, "raw": str(g), "outputs": outputs})
        deadline()
    for i, g in enumerate(samples):
        for h in (g, samples[(i * 37 + 11) % len(samples)]):
            comparisons.append({"notation": "ard_legacy", "a": str(g), "b": str(h),
                                "expected": int(g > h) - int(g < h)})
    ordered = sorted(samples)
    for g, h in zip(ordered, ordered[1:]):
        assert g < h
        comparisons.append({"notation": "ard_legacy", "a": str(g), "b": str(h), "expected": -1})
    ...
    while cursor < len(common) and cursor < 48:
        ...
        for name, value in (("ard_legacy", g), ("rpd", old)):
    ...
    for g, h in zip(common, reversed(common)):
        ...
        for name, left, right in (("ard_legacy", g, h), ("rpd", a, b)):
            comparisons.append({"notation": name, "a": str(left), "b": str(right), "expected": expected})
    counts = [{"notation": "ard_legacy", "raw": str(g), "expected": local_counts(g)}
    ...
    for n, g in enumerate(family):
        ...
        counts.append({"notation": "ard_legacy", "raw": str(g), "expected": expected})
