...
- [CWY Python checks](tests/test_cwy.py): 80 states, 320 geometric-core expansions, 330 root-stretched/bound expansions, 490 prefix checks, 560 round trips, 310 order comparisons and 2 explicit guard refusals; no resource exclusions, 126 queued states unexplored. About 0.16 seconds.
- [NER family checks](tests/cwy_family.mjs): 80 numeric source states, 960 FS/FS_alter/FS_short comparisons with the preserved wY kernel, 320 direct-column comparisons against the independent Python geometric core, 320 Ω-CWY finite-label agreements and 200 self-indexed source/bundle expansions. Also 520 prefixes, 400 order pairs, 320 view checks, 26 full diagram checks and 7 invalid-input refusals. Direct CWY2 expansion/comparison never invoked its display geometry. Partial count output retained exact prefixes and question-mark suffixes.
- The fixed known decrement input `(1,4,15,51)` is included. Numeric comparisons use safe integers; 15 out-of-range/size candidate branches were excluded and remain unknown. This is not unrestricted numeric JS equivalence or a well-ordering proof.
- The final combined run took 1.995 seconds with reported final RSS 81.8 MiB and a 256 MiB Node heap. Test guards include a 45-second total deadline, 768 MiB RSS check, 1.5-second VM-call limit, bounded queues/widths and a separately bounded Python oracle. On this restricted host, Python fixtures were piped into Node because nested Node subprocess creation was refused; the oracle was not skipped. No persistent process was started. The [machine-readable result](tools/cwy-family-validation.json) records the actual run.
- Ω-CWY rebuilt from the included readable modules to the exact pinned SHA-256. All three NER bundles match their supplied working bytes. No browser UI session was newly tested; these checks load the real standalone registrations with host-interface stubs.
