...
namespace OrdinalFormal.ARD.FiniteUnion
...
ARD moves the anchor as well as parent and root. -/
...
theorem generatedSeam_mem_iff (e : Entry) (len b : Nat) (a : Entry) :
    a ∈ generatedSeam e len b ↔
      ...
      a.parent = move e.parent len b e.parent ∧ a.root ≤ move e.parent len b e.parent := by
  ...
  (∃ b, b < n ∧ j = g.length-1 + b*(g.length-1-e.parent) ∧
    ((∃ s ∈ g[g.length-1]?.getD [],
     ...
     (a.row < move e.parent (g.length-1-e.parent) b e.row ∧
      ...
      a.root ≤ move e.parent (g.length-1-e.parent) b e.parent)))

theorem finiteUnion_paper_iff (g : Graph) (e : Entry) (n j : Nat) (a : Entry) :
  ...
  simp only [ordinarySeam_mem_iff, generatedSeam_mem_iff,
    Weakened, moveEntry, List.getLast?_eq_getElem?]
...
end OrdinalFormal.ARD.FiniteUnion
