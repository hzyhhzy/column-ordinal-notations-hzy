...
theorem skyline_sorted (c : Column) : GenericSortedColumns.Sorted compare (skyline c) :=
  GenericSortedColumns.dedup_sorted compare ARD.nat_compare_laws
    (GenericSortedColumns.sort_weakSorted compare ARD.nat_compare_laws _)

theorem normalize_contains {c : Column} {a : Entry} (ha : a ∈ c) :
    a ∈ normalizeColumn compare c := by
  unfold normalizeColumn
  rw [GenericSortedColumns.mem_dedupSorted_iff compare ARD.nat_compare_laws,
...
theorem seam_subset (last : Column) (e : Entry) (he : e ∈ last) (len b : Nat) :
    seam last e len b ⊆ ARD.ordinarySeam last e len b ++ ARD.generatedSeam e len b := by
  intro a ha
  ...
  · unfold predecessor at ha
    ...
    · split at ha
      · rename_i hk
        ...
        exact List.mem_map.mpr ⟨move e.parent len b e.parent,
...
theorem block_aligned (g : Graph) (e : Entry) (he : e ∈ g.getLast?.getD []) (b : Nat) :
    Aligned (block g e b) (ARD.block g e b) := by
  apply aligned_map
  intro i _ a ha
...
theorem expand_aligned (g : Graph) (n : Nat) : Aligned (expand g n) (ARD.expand g n) := by
  cases he : control compare (g.getLast?.getD []) with
  | none => simpa [expand, ARD.expand, he] using Aligned.self (g.take (g.length-1))
  | some e =>
    simpa [expand, ARD.expand, he] using
...
theorem expand_valid {g : Graph} (hg : Valid g) (n : Nat) : Valid (expand g n) := by
  intro j a ha
  exact ARD.expand_valid hg n j a ((expand_aligned g n).entries j ha)
