...
theorem firstColumn_source_unmoved {g : Graph} (hg : Valid g) (e : Entry) :
    firstColumn g e = skyline (g[e.parent]?.getD [] ++
      seam (g.getLast?.getD []) e (g.length-1-e.parent) 0) := by
  unfold firstColumn
  have h := List.map_congr_left (fun a ha => ARD.move_source_cut_eq hg e.parent
...
theorem firstColumn_entry {g : Graph} (hg : Valid g) (e : Entry) {a : Entry}
  ...
  · exact Or.inl (hg e.parent a hs).2.2
  · have hmove : ∀ s : Entry, moveEntry e.parent (g.length-1-e.parent) 0 s = s := by
      intro s
      cases s
      simp [moveEntry, ARD.moveEntry, move]
...
theorem firstColumn_lt {g : Graph} (hg : Valid g) (hcan : Canonical g)
  ...
  have hn := ARD.nonempty_of_control he
  have hl : g.getLast?.getD [] ∈ g := by
    rw [List.getLast?_eq_some_getLast hn]
    exact List.getLast_mem hn
  apply GenericSortedColumns.compare_lt_of_deleted_pivot compare ARD.nat_compare_laws
  ...
  · intro a ha hnew
    apply (Comparison.entryCompare_lt_iff compare ARD.nat_compare_laws.eq_iff a e).mpr
...
theorem expand_zero_lt (g : Graph) (hg : g ≠ []) : CompareLt (expand g 0) g := by
  unfold CompareLt ARD.CompareLt
  rw [expand_zero, ← List.dropLast_eq_take]
  exact RPDDecrease.listCompare_dropLast_lt _ ARD.columnCompare_self g hg
...
theorem expand_lt {g : Graph} (hg : Valid g) (hcan : Canonical g) (hn : g ≠ [])
  ...
  | some e =>
    ...
    | succ n =>
      have hpos : 0 < g.length-1-e.parent := by have := ARD.control_valid hg he; omega
      ...
      rw [hlast] at hf
      have hc := RPDDecrease.listCompare_append_left _ ARD.columnCompare_self g.dropLast
        (firstColumn g e :: tail) [g.getLast hn]
      rw [List.dropLast_concat_getLast hn] at hc
      unfold CompareLt ARD.CompareLt graphCompare
      rw [ht, hc]
