# 五个记号的 Lean 证明 · [English](README.md)

本目录整理 **Y、RPD、LRD、Ω-LRD3、ARD** 在普通经典 Lean 中的良序证明。交付的是源码工程，而不是一批预编译证书。

限制公理体系的论证见另附的[四系统纸面证明](../proofs/paper/well-ordering.zh-CN.md)及 [ARD 纸面证明](../proofs/paper/ard-well-ordering.zh-CN.md)。这些 Lean 定理**不是**对象理论 `KP_ω + 存在不可数序数` 中的形式推导；打印出来的宿主 Lean 公理依赖本身不证明这个元数学上界。

## 主要定理入口

建议先看 [FiveNotationFinalAudit.lean](src/FiveNotationFinalAudit.lean)：它导入原四系统验收、ARD 最终定理及压缩／完整根表示桥，打印十二项公理依赖报告。原 [FourNotationFinalAudit.lean](src/FourNotationFinalAudit.lean) 保持不变。
...
本目录包含 **247 个 Lean 源码模块**：161 个未经修改的上游 Y 模块、86 个本地证明模块。另有 **12 个固定版本的 BMS 模块**由 Lake 作为外部依赖获取。证明源码闭包共 **259 个模块**，此外还依赖 Lean、Mathlib 及 Mathlib 的依赖库。发布内容不需要无关的暂停中对象理论研究、构建缓存、旧记号定义或机器私有绝对路径。
...
已于 **2026-09-13** 验证：全部 **259 个证明源码模块**在发布目录自己的输出目录内从源码重建，**647 项预期公理报告**全部通过。随后通过 `--resume` 再次核对了每个模块的源码／依赖指纹与产物哈希。参见不含机器私有路径的[验收记录](VERIFICATION.json)。

新增真实日志包括 [ARD](verification/ARDFinal.log)（7 项）、[五系统联合验收](verification/FiveNotationFinalAudit.log)（12 项）、[独立规则等价桥](verification/ARDDefinitionFidelity.log)（6 项）、[压缩／完整根表示桥](verification/ARDCompression.log)（5 项）。

原四系统的历史[验收记录](verification/FourNotation-VERIFICATION.json)及日志保持不变：[Y](verification/FiniteDemandYFinal.log)、[RPD](verification/FiniteDemandRPDFinal.log)、[LRD](verification/FiniteDemandLRDFinal.log)、[Ω-LRD3](verification/OmegaLRD3Final.log)、[四系统联合验收](verification/FourNotationFinalAudit.log)。旧日志与本次五系统重建的对应日志按 LF 字节核对，全部完全一致；历史验收记录本身仍只记录原四系统那次检查。九份保留的最终／桥日志合计包含 58 项报告。
