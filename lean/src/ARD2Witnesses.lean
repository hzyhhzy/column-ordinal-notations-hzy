...
def Bad (t : Label.{u} × Label.{u}) (a kappa : Label.{u}) (p : Packet.{u}) : Prop :=
  Input relation (kappa,t) a p.1 p.2 ∧ ¬ ∃ g, Output relation a p.1 p.2 g

theorem exists_bad {t : Label.{u} × Label.{u}} {a kappa : Label.{u}}
    (hg : Guard (kappa,t) a) (hn : ¬ pairRelation t a kappa) : ∃ p, Bad t a kappa p := by
  classical
  have hd : ¬ Demands relation (kappa,t) a := by
    intro hd
    exact hn ((pair_relation_iff t a kappa).mpr ⟨hg,hd⟩)
  simp only [Demands,not_forall,_root_.not_imp] at hd
  obtain ⟨A,f,hf,hn⟩ := hd
  exact ⟨⟨A,f⟩,hf,hn⟩

noncomputable def leastBad (t : Label.{u} × Label.{u}) (a kappa : Label.{u})
    (h : ∃ p, Bad t a kappa p) : Packet.{u} :=
  packetEarlier_wellFounded.min {p | Bad t a kappa p} h

theorem leastBad_spec (t : Label.{u} × Label.{u}) (a kappa : Label.{u})
...
def Extends (R : Rel.{u}) (kappa : Label.{u}) (A : Shape)
  ...
  ARD2.Holds (· < ·) domain R A.graph (labels f) ∧ Bounded (· < ·) A.graph.length (labels f) kappa ∧
  (∀ i : Fin A.cut, labels f i = pref i) ∧ ARD2.Ends R A.graph.length A.needs (labels f) kappa
...
#print axioms ARD2Demand.exists_bad
#print axioms ARD2Demand.leastBad_spec
#print axioms ARD2Demand.badOp_lt
#print axioms ARD2Demand.leastExtension_spec
#print axioms ARD2Demand.extOp_lt
