...
/-! Strict column comparison decrease of the actual dynamic-row ARD2 expansion.
...
theorem move_source_cut_parent {g : Graph} (hg : Valid g) (c len b : Nat)
    {a : Entry} (ha : a ∈ g[c]?.getD []) :
    (moveEntry c len b a).parent = a.parent := by
  have hp := (hg c a ha).2.2
  simp [moveEntry, move, hp]
...
theorem firstColumn_entry {g : Graph} (hg : Valid g) (hclosed : RootClosed g)
  ...
  · obtain ⟨old, hold, rfl⟩ := List.mem_map.mp hs
    have hm := move_source_cut_parent hg e.parent (g.length-1-e.parent) 1 hold
    exact Or.inl (hp.trans hm ▸ (hg e.parent old hold).2.2)
  · rcases List.mem_append.mp hs with hs | hs
...
/-- Every actual ARD2 expansion strictly lowers the column comparator.
...
#print axioms move_source_cut_parent
