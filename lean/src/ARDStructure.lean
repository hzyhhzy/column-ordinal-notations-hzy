...
/-! Finite geometry of the actual ARD expansion. All three stored coordinates
move together; the additional row-before-child invariant is preserved. -/
...
theorem root_le_cut {g : Graph} {e : Entry} (hr : e.root ≤ e.parent) (b : Nat) :
...
theorem seam_valid {g : Graph} (hg : Valid g) (e : Entry)
  ...
  · simp only [ordinarySeam, List.mem_flatMap, List.mem_map, List.mem_filter, List.mem_range] at ha
    ...
    have hv := hg _ s hs
    have hr := ExpansionValidity.move_mono e.parent (g.length-1-e.parent) b hv.2.1
    change move e.parent (g.length-1-e.parent) b s.row < width g e b ∧
      q ≤ move e.parent (g.length-1-e.parent) b s.parent ∧
      move e.parent (g.length-1-e.parent) b s.parent < width g e b
    exact ⟨shift_lt hv.1 b, Nat.le_trans (by omega) hr, shift_lt hv.2.2 b⟩
...
theorem block_valid {g : Graph} (hg : Valid g) (e : Entry)
  ...
  · have hv := hg (e.parent+i) s hs
    have hrow := ExpansionValidity.move_lt_shift e.parent (g.length-1-e.parent) (b+1) hv.1
    have hparent := ExpansionValidity.move_lt_shift e.parent (g.length-1-e.parent) (b+1) hv.2.2
    have hroot := ExpansionValidity.move_mono e.parent (g.length-1-e.parent) (b+1) hv.2.1
    simp only [moveEntry] at hr hp hq
    simp only [Nat.add_mul, Nat.one_mul] at hrow hparent
    unfold NeedValid width
    rw [hr, hp]
    exact ⟨by omega, Nat.le_trans hq hroot, by omega⟩
  · subst i
