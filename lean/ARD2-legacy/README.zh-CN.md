# ARD2-legacy：独立 Lean 子项目 · [English](README.md)

2026-09-17：旧版 21 个数学模块逐字节迁移到此目录，命名空间保留 `OrdinalFormal.ARD2`。在新路径重新验证 **51 模块 / 160 公理报告**，21 个私有模块重新编译、30 个共享模块经源码及指纹核验后复用。默认新版另见[ARD2](../ARD2/README.zh-CN.md)；下方拆分记录为历史说明。

本项目独有 **21 个 Lean 源码模块**，精确证明源码闭包共 **51 个模块**，包括实际导入的共享模块。它不依赖其他记号的私有项目。

主要入口：[ARD2Final.lean](src/ARD2Final.lean)。主要定理：`OrdinalFormal.ARD2.paper_standard_with_top_strictWellOrder`。完整定理范围，以及普通 Lean 与纸面弱集合论证明的区别，见[总说明](../README.zh-CN.md)。
...
2026-09-14 的[独立项目收据](VERIFICATION.json)：**51 个模块、160 项公理报告**通过；其中 21 个模块在本次从源码编译，30 个复用刚刚核验的共享产物。报告只使用普通 Lean 的 `propext`、`Classical.choice`、`Quot.sound` 或其子集；这不是弱 KP 对象理论形式化。
