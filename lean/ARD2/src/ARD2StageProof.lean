import ARD2StageDemands

/-! Actual dynamic ARD2 splicing, arbitrarily many blocks, and the bounded
...
theorem splice_actual_stage
    ...
    (hNeeds : Ends R (width g e b) (seam g e b) reflected (f (cut g e b))) :
  ...
  have hAtReuse : ∀ i, i ≤ g.length-1 →
      atEnd (width g e (b+1)) next beta (shift g e (b+1) i) =
        atEnd (width g e b) f beta (shift g e b i) := by
    intro i hi
    by_cases hlt : i < g.length-1
    · rw [atEnd_before _ _ _ (shift_lt hlt (b+1)),
        atEnd_before _ _ _ (shift_lt hlt b), hReuse i hlt]
    · have heq : i = g.length-1 := by omega
      subst i
      simp only [shift_last he, atEnd_self]
  have hTemplatesNext : Templates R g e (b + 1) next beta := by
    intro a ha
    have hv : a.row ≤ g.length - 1 ∧ a.root ≤ g.length-1 ∧ a.parent < g.length - 1 := by
      rw [List.getLast?_eq_getElem?] at ha
      exact hg _ a ha
    change R (atEnd (width g e (b+1)) next beta (shift g e (b+1) a.row))
      (atEnd (width g e (b+1)) next beta (shift g e (b+1) a.root))
      (next (shift g e (b+1) a.parent)) beta
    rw [hAtReuse a.row hv.1, hAtReuse a.root hv.2.1, hReuse a.parent hv.2.2]
    ...
    · rcases horigin with ⟨s, hs, hw⟩ | ⟨hi0, s, hs, hw⟩
      · have hsource : e.parent + i < g.length - 1 := by omega
        ...
        apply weaken_relation lt R hWeak next (width g e (b + 1)) j hOrder a
          (ARD2.moveEntry e.parent (g.length - 1 - e.parent) (b + 1) s)
...
theorem all_stages
  ...
  | zero =>
    ...
    · intro a ha
      rw [List.getLast?_eq_getElem?] at ha
      have hv := hg (g.length-1) a ha
      change R (atEnd (width g e 0) initial (initial (g.length-1)) (shift g e 0 a.row))
        (atEnd (width g e 0) initial (initial (g.length-1)) (shift g e 0 a.root))
        (initial (shift g e 0 a.parent)) (initial (g.length-1))
      simpa only [width, Nat.zero_mul, Nat.add_zero, shift, move, ite_self,
        atEnd_selfValue (g.length-1) initial hv.1,
        atEnd_selfValue (g.length-1) initial hv.2.1] using
        hInitial.relations (g.length - 1) a ha
  | succ n ih =>
