# IPD：独立 Lean 子项目 · [English](README.md)

本项目独有 **40 个 Lean 源码模块**，精确证明源码闭包共 **55 个模块**，包括实际导入的共享模块。它不依赖其他记号的私有项目。

主要入口：[IPDStandardOrder.lean](src/IPDStandardOrder.lean)。主要定理：`IPD.standard_wellFounded`。完整定理范围，以及普通 Lean 与纸面弱集合论证明的区别，见[总说明](../README.zh-CN.md)。
...
2026-09-14 的[独立项目收据](VERIFICATION.json)：**55 个模块、166 项公理报告**通过；其中 40 个模块在本次从源码编译，15 个复用刚刚核验的共享产物。报告只使用普通 Lean 的 `propext`、`Classical.choice`、`Quot.sound` 或其子集；这不是弱 KP 对象理论形式化。
