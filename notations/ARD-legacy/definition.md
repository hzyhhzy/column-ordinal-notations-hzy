# ARD-legacy: anchored row diagrams · [中文版](definition.zh-CN.md)

These are the preserved legacy rules; the historical name ARD below means ARD-legacy. For the new default see [ARD](../ARD/definition.md).


Release definition, 2026-09-13. [PDF](definition.pdf) · [NER arc-diagram expander](ARD-arcs.ne-rewritten.js) · [Python](ard.py) · [well-ordering proof](../../proofs/paper/ard-legacy-well-ordering.md) · [ordinary Lean entry point](../../lean/ARD-legacy/src/ARDFinal.lean).
...
Import [ARD-arcs.ne-rewritten.js](ARD-arcs.ne-rewritten.js) through NER's custom-notation control and select `ARD-legacy`. Its current registration ID is `ard-adjacency-v01`; the ordinary list is the default, with `计数序列`, `弧线图`, `邻接表（文字）` and `邻接表（图）` in the equivalent-display menu. The script's resource safeguards do not change the finite mathematical rule.
