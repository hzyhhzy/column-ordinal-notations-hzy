# ARD2-legacy: full-context anchored row diagrams · [中文版](definition.zh-CN.md)

2026-09-17: this directory preserves the original full-package rule as **ARD2-legacy**. See [skyline ARD2](../ARD2/definition.md) for the default edition. The historical definition and proof scope below are retained, with archive names and paths updated.

Definition, 2026-09-14. [PDF](definition.pdf) · [NER expander](ARD2-legacy.ne-rewritten.js) · [Python](ard2.py) · [well-ordering proof](../../proofs/paper/ard2-legacy-well-ordering.md) · [ordinary Lean entry point](../../lean/ARD2-legacy/src/ARD2Final.lean).
...
Adjoin a greatest external symbol $\mathsf{Top}$, displayed as `Limit of ARD2-legacy`. It is not a finite column and cannot be used as a coordinate.
...
The [paper](../../proofs/paper/ard2-legacy-well-ordering.md) and [ordinary Lean entry point](../../lean/ARD2-legacy/src/ARD2Final.lean) distinguish two conclusions:
...
The paper's axiom bound is $KP_\omega+\text{“there exists an uncountable ordinal”}$, with full Set Induction and without Power Set or Choice. The actual ordinary Lean proof has compiled; its final axiom reports use only `propext`, `Classical.choice`, and `Quot.sound`. This is **not** a Lean encoding of the weak theory's syntax and an internal derivation in that theory. The [definition-fidelity module](../../lean/ARD2-legacy/src/ARD2DefinitionFidelity.lean) connects the finite paper rule and its reachable domain to the Lean definitions; it is not a proof about a Python or JavaScript virtual machine. See the repository [validation record](../../VALIDATION.md) for verification status.
...
Import [ARD2-legacy.ne-rewritten.js](ARD2-legacy.ne-rewritten.js) through NER's custom-notation control, then select **ARD2-legacy**. The registration ID is `ard2-legacy-v01`; the default display is `列表`. The equivalent-display menu supplies the other four views above. No dependency scripts, network access or persistent cache are required.

Accepted main input includes `A3`, `A3[2][1]`, `Limit[3]`, `Limit of ARD2-legacy`, a natural number, and a full triple list. Whitespace is ignored. Natural numbers mean that many empty columns, not a count-sequence encoding. The three NER fundamental-sequence choices `FS`, `FS_alter` and `FS_short` use exactly the same rule with no index offset.
...
The preserved [legacy Python tests](../../tests/test_ard2_legacy.py) cover full-root closure, the two SELF rebindings, lower-row generation, prefixes, comparison and exact small counts. The current [NER rule tests](../../tests/ard2_ner.cjs) also load both registrations and check the legacy seeds against the skyline projection; the [display tests](../../tests/ard2_display.cjs) target the new default edition. These bounded tests do not replace the proof. From the repository root:
